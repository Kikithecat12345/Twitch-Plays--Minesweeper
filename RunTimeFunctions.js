...
function timerTick(){
  ...
  if(LastSelectedDay == 23){
    if(GameState == 2) Day23.fnct.death();
    if(GameState == 3) Day23.fnct.win();
  }
}
...
function leftClick(posX, posY){
  ...
  if((LastSelectedDay == 23) && Day23.fnct.leftClick(posX, posY)) return;
  if(!hasBeenGenerated){
    generate(posX, posY);
...
function rightClick(posX, posY){
  ...
  if((LastSelectedDay == 23) && Day23.fnct.rightClick(posX, posY)) return;
  if(field[posX][posY].open){
